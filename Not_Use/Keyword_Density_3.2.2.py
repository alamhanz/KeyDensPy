...
def main():
    ...
    cl=list(r.columns.values)
    
    #test1
    r_lis=list(r.reset_index().values)
    r_m=np.array(r_lis)
    ...
    for w in r22:
        try :
            twk.append(word_tokenize(str(w)))
        except:
            print w
    t=time()-t1
    print t    
    ...
    #for i in bar(range(len(r2))):
    #    sleep(0.02)
    #    try :
    #        tw.append(word_tokenize(str(r2[i])))
    #    except:
    #        u=u+1
    #        tw.append(['nan'])
    t=time()-t1
    ...
    tw3=pd.DataFrame(pd.Series(tw),columns=[('test')])
    tw3=tw3.loc[tw3['test'].isin(wo)]
    #wo=tw3.merge(tw2,on=['test'])
    #wut=tw3[(~tw3.test.isin(wo.test))]
